...
fn fs_main(input: VertexOutput) -> @location(0) vec4<f32> {
    ...
    let depth = textureLoad(depthTexture, screenPos, 0);
    
    // 1. RECONSTRUCCIÓN PERFECTA NDC: Y invertido correctamente para WebGPU
    ...
    // 2. NORMALES AAA: Calculamos cross(dy, dx) en lugar de (dx, dy) 
    // para obligar a que el vector apunte HACIA AFUERA de la geometría y no hacia adentro.
    ...
    var occlusion = 0.0;
    let radius = 25.0; // Expandimos el radio un poco para que se note en las esquinas
    let samples = 16.0;
    ...
    let noise = hash(input.position.xy) * 6.2831853; 
    ...
    for (var i = 0u; i < 16u; i = i + 1u) {
        ...
        let sampleDir = tbn * vec3<f32>(x, y, z);
        
        // 3. NORMAL BIAS: Empujamos el rayo 2cm hacia afuera del objeto antes de disparar 
        // para evitar el temible acné de piso (Self-Shadowing).
        let samplePos = worldPos + (worldNormal * 2.0) + (sampleDir * radius);

        let clipPos = env.viewProjectionMatrix * vec4<f32>(samplePos, 1.0);
        
        // 4. PROTECCIÓN DE LENTE: Si el rayo cruza hacia atrás de la cámara, ignorarlo.
        if (clipPos.w <= 0.01) { continue; }

        let sampleNDC = clipPos.xyz / clipPos.w;
        let sampleUV = sampleNDC.xy * 0.5 + 0.5;
        let finalUV = vec2<f32>(sampleUV.x, 1.0 - sampleUV.y);

        if (finalUV.x >= 0.0 && finalUV.x <= 1.0 && finalUV.y >= 0.0 && finalUV.y <= 1.0) {
            let texDim = vec2<f32>(textureDimensions(depthTexture));
            let sampleScreenPos = vec2<i32>(finalUV * texDim);
            
            let sceneDepth = textureLoad(depthTexture, sampleScreenPos, 0);
            
            // Tolerancia fina para contacto directo
            if (sceneDepth < sampleNDC.z - 0.0001) { 
                let sceneNdc = vec4<f32>(sampleNDC.x, sampleNDC.y, sceneDepth, 1.0);
                let sceneUnp = invVP * sceneNdc;
                let sceneWorldPos = sceneUnp.xyz / sceneUnp.w;
    ...
    let aoRaw = max(0.0, 1.0 - (occlusion / samples));
    
    // Elevamos al poder para dar contraste profundo a las grietas (Estilo Unreal)
    let aoFinal = pow(aoRaw, 1.5);
