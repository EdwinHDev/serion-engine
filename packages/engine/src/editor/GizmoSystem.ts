...
export class GizmoSystem {
  ...
  public getRotationReadout(): string {
    ...
    const sign = deg < 0 ? '-' : '';
    let text = `${sign}${absDeg.toFixed(2)}°`;
    if (turns > 0) text += ` (${turns} vueltas)`;
    return text;
  }
  ...
  private rebuildGeometry(): void {
    ...
    if (this.mode === 'translate') {
      ...
      const addRuler = (axis: 'x'|'y'|'z', radius: number, snapDeg: number, color: number[]) => {
        ...
        const tickLen = 0.3; const halfW = 0.015;
      ...
      if (this.isDragging && this.hoveredPart) {
        ...
        const wedgeRadius = this.ROT_RADIUS * 0.8; 
  ...
  public beginDrag(ray: Ray, camera: SCamera, actor: SActor, snapEnabled: boolean = false, snapValue: number = 0): void {
     ...
     if (t !== null) {
       ...
       if (this.mode === 'translate') {
         ...
         this.dragStartAngle = Math.atan2(v, u);
         this.dragLastAngle = this.dragStartAngle; // Guardar el ángulo base para el delta
         this.accumulatedAngle = 0; // Reiniciar acumulador continuo
         this.currentDeltaAngle = 0;
       }
     }
